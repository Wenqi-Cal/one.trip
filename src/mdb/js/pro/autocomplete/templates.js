...
export function getDropdownTemplate(settings) {
  ...
  itemsList.setAttribute('aria-label', options.itemsListLabel);
  itemsList.setAttribute('tabindex', '0');
...
export function getNoResultsTemplate(message) {
  return `<li role="option" aria-disabled="true" class="${CLASS_NAME_NO_RESULTS}">${message}</li>`;
}
