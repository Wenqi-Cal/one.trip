...
const IMAGE_WRAPPER_SELECTOR = 'data-mdb-image-wrapper';
...
class EcommerceGallery {
  constructor(element, options = {}) {
    ...
    this._onMainImgMousemove = this._onMainImgMousemove.bind(this);
    this._onMainImgMouseleave = this._onMainImgMouseleave.bind(this);
  ...
  _addEvents() {
    ...
    if (this.options.zoomEffect) {
      const imageWrapper = SelectorEngine.findOne(`[${IMAGE_WRAPPER_SELECTOR}]`, this._element);
      EventHandler.on(imageWrapper, 'mousemove', this._onMainImgMousemove);
      EventHandler.on(imageWrapper, 'mouseleave', this._onMainImgMouseleave);
    }
  }
  ...
  _removeEvents() {
    ...
    if (this.options.zoomEffect) {
      const imageWrapper = SelectorEngine.findOne(`[${IMAGE_WRAPPER_SELECTOR}]`, this._element);
      EventHandler.off(imageWrapper, 'mousemove', this._onMainImgMousemove);
    }
  }
  ...
  _onMainImgMousemove(e) {
    ...
    const cursorPosition = {
      x: Math.max(
        0,
        Math.min(e.clientX - imageWrapperRect.left - paddingLeft, imageWrapperRect.width)
      ),
      y: Math.max(
        0,
        Math.min(e.clientY - imageWrapperRect.top - paddingTop, imageWrapperRect.height)
      ),
    };

    const imageCenter = {
      x: this._activeImg.width / 2,
      y: this._activeImg.height / 2,
    };

    const positionDiff = {
      x: (imageCenter.x - cursorPosition.x) * (scale - 1),
      y: (imageCenter.y - cursorPosition.y) * (scale - 1),
    };

    Manipulator.style(this._activeImg, {
      transform: `translate(${positionDiff.x}px,${positionDiff.y}px) scale(${scale})`,
    });
  }
