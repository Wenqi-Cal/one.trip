...
class FileUpload {
  ...
  _createDefaultFilePreview() {
    ...
    const isKnownImageExtension = ['jpg', 'jpeg', 'png', 'svg', 'webp', 'bmp', 'gif'].includes(
    ...
    if (isKnownImageExtension) {
      ...
      // Render a generic file icon as a fallback immediately
      ...
      Manipulator.addClass(renderContainer, 'file-upload-file-icon-wrapper');

      // Attempt to detect if the provided URL is actually an image (even without extension)
      // If it is, replace the fallback with an image preview once loaded
      try {
        const testImg = new Image();
        testImg.className = 'file-upload-preview-img';
        testImg.onload = () => {
          Manipulator.removeClass(renderContainer, 'file-upload-file-icon-wrapper');
          // Replace any existing children (fallback icon) with the loaded image
          // Using replaceChildren for simplicity and to avoid duplicates
          if (renderContainer && renderContainer.replaceChildren) {
            renderContainer.replaceChildren(testImg);
          } else {
            // Fallback for environments without replaceChildren
            renderContainer.innerHTML = '';
            renderContainer.appendChild(testImg);
          }
        };
        testImg.onerror = () => {
          // Do nothing, keep the generic file icon
        };
        // Trigger loading
        testImg.src = this.options.defaultFile;
      } catch (e) {
        // In case of any unexpected errors, keep the generic file icon
      }
    }
