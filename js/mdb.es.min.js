...
const NAME$W = "button";
const DATA_KEY$E = `mdb.${NAME$W}`;
const EVENT_KEY$w = `.${DATA_KEY$E}`;
const EVENT_CLICK$3 = `click${EVENT_KEY$w}`;
const EVENT_TRANSITIONEND = "transitionend";
const EVENT_MOUSEENTER$2 = "mouseenter";
const EVENT_MOUSELEAVE$2 = "mouseleave";
const EVENT_HIDE$a = `hide${EVENT_KEY$w}`;
const EVENT_HIDDEN$b = `hidden${EVENT_KEY$w}`;
const EVENT_SHOW$b = `show${EVENT_KEY$w}`;
const EVENT_SHOWN$a = `shown${EVENT_KEY$w}`;
...
class Button2 extends Button$1 {
  constructor(element2) {
    ...
    this._fn = {};
    if (this._element) {
      Data$1.setData(this._element, DATA_KEY$E, this);
      this._init();
  ...
  show() {
    if (Manipulator$1.hasClass(this._element, CLASS_NAME_FIXED_ACTION_BTN)) {
      ...
      EventHandler$1.trigger(this._element, EVENT_SHOW$b);
  ...
  hide() {
    if (Manipulator$1.hasClass(this._element, CLASS_NAME_FIXED_ACTION_BTN)) {
      EventHandler$1.off(this._buttonList, EVENT_TRANSITIONEND);
      EventHandler$1.trigger(this._element, EVENT_HIDE$a);
  ...
  _bindListHideTransitionEnd() {
    EventHandler$1.on(this._buttonList, EVENT_TRANSITIONEND, (event) => {
      if (event.propertyName === "transform") {
        ...
        EventHandler$1.trigger(this._element, EVENT_HIDDEN$b);
      }
    });
  }
  _bindListOpenTransitionEnd() {
    EventHandler$1.on(this._buttonList, EVENT_TRANSITIONEND, (event) => {
      if (event.propertyName === "transform") {
        EventHandler$1.off(this._buttonList, EVENT_TRANSITIONEND);
        EventHandler$1.trigger(this._element, EVENT_SHOWN$a);
      }
    });
  }
...
const DATA_KEY$D = "bs.focustrap";
const EVENT_KEY$v = `.${DATA_KEY$D}`;
const EVENT_FOCUSIN$2 = `focusin${EVENT_KEY$v}`;
const EVENT_KEYDOWN_TAB = `keydown.tab${EVENT_KEY$v}`;
const TAB_KEY$1 = "Tab";
...
let FocusTrap$1 = class FocusTrap extends Config {
  ...
  activate() {
    ...
    EventHandler.off(document, EVENT_KEY$v);
...
const EVENT_SHOW$a = `show${EVENT_KEY$u}`;
const EVENT_SHOWN$9 = `shown${EVENT_KEY$u}`;
const EVENT_HIDE$9 = `hide${EVENT_KEY$u}`;
const EVENT_HIDE_PREVENTED$1 = `hidePrevented${EVENT_KEY$u}`;
const EVENT_HIDDEN$a = `hidden${EVENT_KEY$u}`;
const EVENT_KEYDOWN_DISMISS$1 = `keydown.dismiss${EVENT_KEY$u}`;
const Default$x = {
  backdrop: true,
  keyboard: true,
  scroll: false
...
class Offcanvas extends BaseComponent$1 {
  ...
  show(relatedTarget) {
    ...
    const showEvent = EventHandler.trigger(this._element, EVENT_SHOW$a, { relatedTarget });
    ...
    const completeCallBack = () => {
      ...
      EventHandler.trigger(this._element, EVENT_SHOWN$9, { relatedTarget });
    };
  ...
  hide() {
    ...
    const hideEvent = EventHandler.trigger(this._element, EVENT_HIDE$9);
...
const NAME$S = "swipe";
const EVENT_KEY$t = ".bs.swipe";
const EVENT_TOUCHSTART = `touchstart${EVENT_KEY$t}`;
const EVENT_TOUCHMOVE = `touchmove${EVENT_KEY$t}`;
const EVENT_TOUCHEND = `touchend${EVENT_KEY$t}`;
const EVENT_POINTERDOWN = `pointerdown${EVENT_KEY$t}`;
const EVENT_POINTERUP = `pointerup${EVENT_KEY$t}`;
...
let Swipe$2 = class Swipe extends Config {
  ...
  // Public
  dispose() {
    EventHandler.off(this._element, EVENT_KEY$t);
  }
  // Private
  _start(event) {
...
const EVENT_SLIDE = `slide${EVENT_KEY$s}`;
const EVENT_SLID = `slid${EVENT_KEY$s}`;
const EVENT_KEYDOWN$3 = `keydown${EVENT_KEY$s}`;
const EVENT_MOUSEENTER$1 = `mouseenter${EVENT_KEY$s}`;
const EVENT_MOUSELEAVE$1 = `mouseleave${EVENT_KEY$s}`;
const EVENT_DRAG_START = `dragstart${EVENT_KEY$s}`;
...
const EVENT_HIDE$8 = "hide";
const EVENT_HIDDEN$9 = "hidden";
const EVENT_SHOW$9 = "show";
const EVENT_SHOWN$8 = "shown";
...
let Tooltip$1 = class Tooltip extends BaseComponent$1 {
  ...
  show() {
    ...
    const showEvent = EventHandler.trigger(this._element, this.constructor.eventName(EVENT_SHOW$9));
    ...
    const complete = () => {
      EventHandler.trigger(this._element, this.constructor.eventName(EVENT_SHOWN$8));
  ...
  hide() {
    ...
    const hideEvent = EventHandler.trigger(this._element, this.constructor.eventName(EVENT_HIDE$8));
    if (hideEvent.defaultPrevented) {
      return;
    }
    const tip = this._getTipElement();
    ...
    const complete = () => {
      ...
      EventHandler.trigger(this._element, this.constructor.eventName(EVENT_HIDDEN$9));
    };
    this._queueCallback(complete, this.tip, this._isAnimated());
  }
  update() {
...
const EVENT_ACTIVATE$1 = `activate${EVENT_KEY$r}`;
const EVENT_CLICK$1 = `click${EVENT_KEY$r}`;
...
const NAME$J = "tab";
const DATA_KEY$y = "bs.tab";
const EVENT_KEY$p = `.${DATA_KEY$y}`;
const EVENT_HIDE$7 = `hide${EVENT_KEY$p}`;
const EVENT_HIDDEN$8 = `hidden${EVENT_KEY$p}`;
const EVENT_SHOW$8 = `show${EVENT_KEY$p}`;
const EVENT_SHOWN$7 = `shown${EVENT_KEY$p}`;
const EVENT_KEYDOWN$2 = `keydown${EVENT_KEY$p}`;
...
const SELECTOR_DATA_TOGGLE$5 = "[data-mdb-tab-initialized]";
const SELECTOR_INNER_ELEM = `${SELECTOR_INNER}, ${SELECTOR_DATA_TOGGLE$5}`;
let Tab$1 = class Tab extends BaseComponent$1 {
  ...
  show() {
    ...
    const hideEvent = active ? EventHandler.trigger(active, EVENT_HIDE$7, { relatedTarget: innerElem }) : null;
    const showEvent = EventHandler.trigger(innerElem, EVENT_SHOW$8, { relatedTarget: active });
  ...
  _activate(element2, relatedElem) {
    ...
    const complete = () => {
      ...
      EventHandler.trigger(element2, EVENT_SHOWN$7, {
  ...
  _deactivate(element2, relatedElem) {
    ...
    const complete = () => {
      ...
      EventHandler.trigger(element2, EVENT_HIDDEN$8, { relatedTarget: relatedElem });
    };
    this._queueCallback(complete, element2, element2.classList.contains(CLASS_NAME_FADE$4));
  }
  _keydown(event) {
...
class Tab2 extends Tab$1 {
  ...
  show() {
    ...
    if (active) {
      ...
      hideEventMdb = EventHandler$1.trigger(active, EVENT_HIDE$6, { relatedTarget: innerElem });
    }
    const showEvent = EventHandler$1.trigger(innerElem, EVENT_SHOW_BS$3, { relatedTarget: active });
    const showEventMdb = EventHandler$1.trigger(innerElem, EVENT_SHOW$7, { relatedTarget: active });
  ...
  _activate(element2, relatedElem) {
    ...
    const complete = () => {
      ...
      EventHandler$1.trigger(element2, EVENT_SHOWN$6, {
        relatedTarget: relatedElem
      });
    };
  ...
  _deactivate(element2, relatedElem) {
    ...
    const complete = () => {
      ...
      EventHandler$1.trigger(element2, EVENT_HIDDEN$7, { relatedTarget: relatedElem });
    };
    this._queueCallback(complete, element2, element2.classList.contains(CLASS_NAME_FADE$3));
  }
}
...
class Input extends BaseComponent2 {
  ...
  static jQueryInterface(config, options) {
    return this.each(function() {
      let data = Data$1.getData(this, DATA_KEY$w);
      const _config = typeof config === "object" && config;
      if (!data && /dispose/.test(config)) {
        return;
      }
...
const DATA_KEY$v = "bs.collapse";
const EVENT_KEY$n = `.${DATA_KEY$v}`;
...
const SELECTOR_DATA_TOGGLE$4 = "[data-mdb-collapse-init]";
const Default$q = {
  parent: null,
  toggle: true
};
...
const SELECTOR_DATA_TOGGLE$3 = "[data-mdb-dropdown-initialized]:not(.disabled):not(:disabled)";
const SELECTOR_DATA_TOGGLE_SHOWN = `${SELECTOR_DATA_TOGGLE$3}.${CLASS_NAME_SHOW$3}`;
...
const EVENT_HIDE$3 = "hide.bs.dropdown";
const EVENT_HIDDEN$4 = "hidden.bs.dropdown";
const EVENT_SHOW$4 = "show.bs.dropdown";
const EVENT_SHOWN$3 = "shown.bs.dropdown";
const EVENT_HIDE_MDB = `hide${EVENT_KEY$l}`;
const EVENT_HIDDEN_MDB = `hidden${EVENT_KEY$l}`;
const EVENT_SHOW_MDB = `show${EVENT_KEY$l}`;
const EVENT_SHOWN_MDB = `shown${EVENT_KEY$l}`;
...
class Dropdown2 extends Dropdown$1 {
  ...
  dispose() {
    EventHandler$1.off(this._element, EVENT_SHOW$4);
    EventHandler$1.off(this._parent, EVENT_SHOWN$3);
    EventHandler$1.off(this._parent, EVENT_HIDE$3);
    EventHandler$1.off(this._parent, EVENT_HIDDEN$4);
    Manipulator$1.removeDataAttribute(this._element, `${this.constructor.NAME}-initialized`);
    super.dispose();
  }
  // Getters
  ...
  _bindShowEvent() {
    EventHandler$1.on(this._element, EVENT_SHOW$4, (e) => {
  ...
  _bindShownEvent() {
    EventHandler$1.on(this._parent, EVENT_SHOWN$3, (e) => {
  ...
  _bindHideEvent() {
    EventHandler$1.on(this._parent, EVENT_HIDE$3, (e) => {
  ...
  _bindHiddenEvent() {
    EventHandler$1.on(this._parent, EVENT_HIDDEN$4, (e) => {
...
class Ripple extends BaseComponent2 {
  ...
  static jQueryInterface(options) {
    return this.each(function() {
      const data = Data$1.getData(this, DATA_KEY$s);
      if (!data) {
...
class Range extends BaseComponent2 {
  ...
  static jQueryInterface(config, options) {
    return this.each(function() {
      let data = Data$1.getData(this, DATA_KEY$r);
      const _config = typeof config === "object" && config;
      if (!data && /dispose/.test(config)) {
        return;
      }
...
const DATA_KEY$q = "mdb.lightbox";
const EVENT_KEY$k = `.${DATA_KEY$q}`;
const DATA_API_KEY$2 = ".data-api";
const EVENT_CLICK_DATA_API$2 = `click${EVENT_KEY$k}${DATA_API_KEY$2}`;
...
class Lightbox extends BaseComponent2 {
  ...
  dispose() {
    ...
    Data$1.removeData(this._element, DATA_KEY$q);
    Manipulator$1.removeDataAttribute(this._element, `${this.constructor.NAME}-initialized`);
    super.dispose();
  }
  // Private
  ...
  _updatePreloadedImages(direction) {
    ...
    const targetChild = isDirectionRight ? this._galleryContent.firstChild : this._galleryContent.lastChild;
    const targetImage = targetChild == null ? void 0 : targetChild.querySelector("img");
    ...
    this._preloadedImages = SelectorEngine$1.find("img", this._galleryContent);
  }
  ...
  _zoomOnWheel(x, y, delta) {
    ...
    Manipulator$1.style(this.currentImg, { transition: "" });
    Manipulator$1.style(this.currentImg, {
...
const EVENT_SELECT$3 = `scoreSelect${EVENT_KEY$j}`;
const EVENT_HOVER = `scoreHover${EVENT_KEY$j}`;
const EVENT_SCORE_MOVE = `scoreMove${EVENT_KEY$j}`;
const EVENT_KEYUP$1 = `keyup${EVENT_KEY$j}`;
const EVENT_FOCUSOUT$1 = `focusout${EVENT_KEY$j}`;
const EVENT_KEYDOWN$1 = `keydown${EVENT_KEY$j}`;
const EVENT_MOUSEDOWN$1 = `mousedown${EVENT_KEY$j}`;
class Rating extends BaseComponent2 {
  ...
  dispose() {
    if (!this._options.readonly) {
      ...
      this._icons.forEach((el, i) => {
        ...
        EventHandler$1.off(el, "mousemove");
    ...
    this._icons.forEach((el, i) => {
      el.addEventListener(
        ...
        this._fn.mouseenter[i] = (e) => {
          this._index = this._calculateIndex(e, el, i);
          this._updateRating(this._index);
  ...
  _bindMouseMove() {
    ...
    this._fn.mousemove = [];
    this._icons.forEach((el, i) => {
      EventHandler$1.on(el, "mousemove", (e) => {
        this._index = this._calculateIndex(e, el, i);
        this._updateRating(this._index);
        this._triggerEvents(el, EVENT_SCORE_MOVE);
      });
    });
  }
  _calculateIndex(e, el, i) {
    if (this._options.decimal) {
      const rect = el.getBoundingClientRect();
      const mouseX = e.pageX - rect.left + window.scrollX;
      const elementWidth = el.offsetWidth;
      const fraction = mouseX / elementWidth - 1;
  ...
  _setContentVariable() {
    this._icons.forEach((el) => {
      ...
      el.style.setProperty("--content", `${beforeContent}`);
    });
  }
  ...
  _updateRating(index) {
    ...
    if (this._options.decimal && !Number.isInteger(index)) {
      const partial = Math.floor(index) + 1;
      const fillFraction = this._roundToPrecision(index - Math.floor(index));
      const fillPercentage = `${fillFraction * 100}%`;
      this._index = Number(this._roundToPrecision(index));
      (_a = this._icons[partial]) == null ? void 0 : _a.classList.add("fas", "partial");
      (_b = this._icons[partial]) == null ? void 0 : _b.classList.remove("far");
      (_c = this._icons[partial]) == null ? void 0 : _c.style.setProperty("--percentage", fillPercentage);
    }
  }
  _roundToPrecision(num) {
    const precision = this._options.decimalPrecision;
    if (!precision) {
      return num;
    }
    const decimalDigits = this._countDecimals(precision);
    const roundedNumber = Math.round(num / precision) * precision;
    return roundedNumber.toFixed(decimalDigits);
  }
  _countDecimals(number) {
    const str = number.toFixed(5);
  ...
  _clearRating() {
    this._icons.forEach((el, i) => {
      if (this._options.dynamic) {
        ...
        el.classList.remove("fas", "active", "partial");
...
const elMatches = typeof Element !== "undefined" && (Element.prototype.matches || Element.prototype.webkitMatchesSelector || Element.prototype.mozMatchesSelector || Element.prototype.msMatchesSelector);
...
function queryChildren(element2, selector) {
  return Array.prototype.filter.call(element2.children, (child) => matches(child, selector));
}
const cls = {
  main: "ps",
  rtl: "ps__rtl",
...
function processScrollDiff(i, axis, diff, useScrollingClass = true, forceFireReachEvent = false) {
  let fields;
  if (axis === "top") {
...
function processScrollDiff$1(i, diff, [contentHeight, containerHeight, scrollTop, y, up, down], useScrollingClass = true, forceFireReachEvent = false) {
...
function updateGeometry(i) {
  ...
  if (!element2.contains(i.scrollbarXRail)) {
    queryChildren(element2, cls.element.rail("x")).forEach((el) => remove(el));
    element2.appendChild(i.scrollbarXRail);
  }
  if (!element2.contains(i.scrollbarYRail)) {
    queryChildren(element2, cls.element.rail("y")).forEach((el) => remove(el));
...
function bindMouseScrollHandler(i, [
  ...
  function moveHandler(e) {
    ...
    if (activeSlider === scrollbarAxis) {
      element2[scrollAxis] = startingScrollPosition + scrollBy * (e[pageAxis] - startingMousePagePosition);
  ...
  function bindMoves(e) {
    if (activeSlider === null) {
      ...
      startingScrollPosition = element2[scrollAxis];
      if (e.touches) {
        e[pageAxis] = e.touches[0][`page${axis.toUpperCase()}`];
      }
...
function keyboard(i) {
  const element2 = i.element;
  const elementHovered = () => matches(element2, ":hover");
  const scrollbarFocused = () => matches(i.scrollbarX, ":focus") || matches(i.scrollbarY, ":focus");
...
class PerfectScrollbar {
  constructor(element2, userSettings = {}) {
    ...
    const focus = () => element2.classList.add(cls.state.focus);
    const blur = () => element2.classList.remove(cls.state.focus);
...
class Sidenav extends BaseComponent2 {
  ...
  dispose() {
    ...
    this._touch.dispose();
    Data$1.removeData(this._element, DATA_KEY$o);
    Manipulator$1.removeDataAttribute(this._element, `${this.constructor.NAME}-initialized`);
    super.dispose();
  }
  hide() {
...
const EVENT_KEY$i = `.${DATA_KEY$n}`;
const EVENT_CLOSE$5 = `close${EVENT_KEY$i}`;
const EVENT_CLOSED$1 = `closed${EVENT_KEY$i}`;
...
let Alert$1 = class Alert extends BaseComponent$1 {
  ...
  // Public
  close() {
    const closeEvent = EventHandler.trigger(this._element, EVENT_CLOSE$5);
...
const DATA_KEY$m = `mdb.${NAME$t}`;
const EVENT_KEY$h = `.${DATA_KEY$m}`;
const EVENT_CLOSE_BS = "close.bs.alert";
const EVENT_CLOSED_BS = "closed.bs.alert";
const EVENT_HIDE$2 = `hide${EVENT_KEY$h}`;
const EVENT_HIDDEN$3 = `hidden${EVENT_KEY$h}`;
const EVENT_SHOW$3 = `show${EVENT_KEY$h}`;
const EVENT_SHOWN$2 = `shown${EVENT_KEY$h}`;
...
class Alert2 extends Alert$1 {
  ...
  hide() {
    const hideEvent = EventHandler$1.trigger(this._element, EVENT_HIDE$2);
  ...
  _init() {
    if (this._options.hidden) {
      ...
      Manipulator$1.removeClass(this._element, "show");
      this._isOpen = false;
    }
    this._bindMdbEvents();
    this._setup();
  ...
  _setupStacking() {
    ...
    EventHandler$1.on(this._element, EVENT_CLOSED_BS, () => {
      this._updateAlertStack();
    });
  }
  _setColor() {
...
class Toast2 extends Toast$1 {
  ...
  _setupAlignment() {
    ...
    const oppositeY = ((_a = this.position) == null ? void 0 : _a.y) === "top" ? "bottom" : "top";
    const oppositeX = ((_b = this.position) == null ? void 0 : _b.x) === "left" ? "right" : "left";
    if (((_c = this.position) == null ? void 0 : _c.x) === "center") {
      Manipulator$1.style(this._element, {
      ...
      Manipulator$1.style(this._element, {
        [((_d = this.position) == null ? void 0 : _d.y) || "defaultY"]: `${this.verticalOffset + ((_e = this._config) == null ? void 0 : _e.offset)}px`,
        [((_f = this.position) == null ? void 0 : _f.x) || "defaultX"]: `${(_g = this._config) == null ? void 0 : _g.offset}px`,
        [oppositeY]: "unset",
        [oppositeX]: "unset",
        transform: "unset"
      });
  ...
  const normalTemplate = `<div id='${pickerId}' class='timepicker-wrapper h-100 d-flex align-items-center justify-content-center flex-column position-fixed'>
      <div class="d-flex align-items-center justify-content-center flex-column timepicker-container timepicker-container-${id}">
        <div class="d-flex flex-column timepicker-elements justify-content-around">
          ${createHead(headId, format24, amLabel, pmLabel, okLabel, inline, isRTL2)}
          ${createTimepickerClockWrapper(bodyId, format24)}
        </div>
        ${createFooter$1(footerId, showClearBtn, clearLabel, cancelLabel, okLabel)}
...
function createFooter$1(footerId, showClearBtn, clearLabel, cancelLabel, okLabel) {
  return `
  <div id='${footerId}' class='timepicker-footer'>
    <div class="w-100 d-flex justify-content-between">
      ${showClearBtn ? `<button type='button' class='timepicker-button timepicker-clear' tabindex="0" aria-label="${clearLabel}">${clearLabel}</button>` : ""}
...
function createModeWrapper(inline, format24, amLabel, pmLabel, okLabel) {
  ...
  if (format24) {
    ...
    return `
      <div class="d-flex ${!inline ? "flex-column " : ""}justify-content-center timepicker-mode-wrapper">
        <button type='button' class="timepicker-hour-mode timepicker-am${inline ? " me-2 ms-4" : ""}" tabindex="0">${amLabel}</button>
        <button type='button' class="timepicker-hour-mode timepicker-pm" tabindex="0">${pmLabel}</button>
        ${inline ? `${submitBtn}` : ""}
      </div>`;
  }
}
function createHead(headId, format24, amLabel, pmLabel, okLabel, inline, isRTL2) {
  return `
  <div id='${headId}' class='timepicker-head d-flex flex-row align-items-center justify-content-center ${inline ? "timepicker-head-inline" : ""}'
  style='padding-${isRTL2 ? "left" : "right"}:${format24 && !inline ? 50 : 0}px'>
...
const EVENT_CLOSE$4 = `close${EVENT_KEY$f}`;
const EVENT_OPEN$4 = `open${EVENT_KEY$f}`;
...
const SELECTOR_DATA_TOGGLE$2 = "[data-mdb-toggle]";
...
class Timepicker extends BaseComponent2 {
  ...
  get container() {
    return SelectorEngine$1.findOne(`${SELECTOR_CONTAINER}-${this._toggleButtonId}`);
  }
  get toggleButton() {
    return SelectorEngine$1.findOne("[data-mdb-toggle]", this._element);
  }
  get customIcon() {
    return SelectorEngine$1.findOne(".timepicker-toggle-button", this._element);
  }
  // Public
  init() {
    ...
    let hoursFormat;
    let _amOrPm;
    Manipulator$1.addClass(this._input, INPUT_CLASS);
  ...
  close() {
    const closeEvent = EventHandler$1.trigger(this._element, EVENT_CLOSE$4);
  ...
  open() {
    ...
    const openEvent = EventHandler$1.trigger(this._element, EVENT_OPEN$4);
    if (this._isOpen || openEvent.defaultPrevented) {
      return;
    }
    const container = this._getContainer();
    Manipulator$1.addStyle(this.toggleButton, {
      pointerEvents: "none"
    });
    let checkInputValue;
    ...
    div2.innerHTML = getTimepickerTemplate(this._options, this._toggleButtonId);
    Manipulator$1.addClass(div2, MODAL_CLASS$1);
    Manipulator$1.addClass(div2, `${MODAL_CLASS$1}-${this._toggleButtonId}`);
    ...
    if (!inline) {
      ...
      this._popper = createPopper(this._input, div2, {
  ...
  _removeInputAndToggleListeners() {
    ...
    EventHandler$1.off(this._element, EVENT_CLICK_DATA_API$1, SELECTOR_DATA_TOGGLE$2);
    EventHandler$1.off(this._element, "keydown", SELECTOR_DATA_TOGGLE$2);
  }
  _appendToggleButton() {
  ...
  _getDomElements() {
    this._modal = SelectorEngine$1.findOne(`.${MODAL_CLASS$1}-${this._toggleButtonId}`);
    this._hour = SelectorEngine$1.findOne(`.${HOUR_CLASS}`, this._modal);
    this._minutes = SelectorEngine$1.findOne(`.${MINUTE_CLASS}`, this._modal);
    this._AM = SelectorEngine$1.findOne(`.${AM_CLASS}`, this._modal);
    this._PM = SelectorEngine$1.findOne(`.${PM_CLASS}`, this._modal);
    this._wrapper = SelectorEngine$1.findOne(`.${WRAPPER_CLASS}`, this._modal);
    this._hand = SelectorEngine$1.findOne(`.${HAND_CLASS}`, this._modal);
    this._circle = SelectorEngine$1.findOne(`.${CIRCLE_CLASS}`, this._modal);
    this._clock = SelectorEngine$1.findOne(`.${CLOCK_CLASS}`, this._modal);
    this._clockInner = SelectorEngine$1.findOne(`.${CLOCK_INNER_CLASS}`, this._modal);
  }
  ...
  _listenToToggleKeydown() {
    EventHandler$1.on(this._element, "keydown", SELECTOR_DATA_TOGGLE$2, (e) => {
      if (e.keyCode === ENTER) {
        this.open();
      }
    });
  ...
  _handleOpen() {
    EventHandler$1.on(this._element, EVENT_CLICK_DATA_API$1, SELECTOR_DATA_TOGGLE$2, () => {
      if (this._options === null) {
        return;
      }
      const fixForInput = Manipulator$1.getDataAttribute(this._input, "toggle") !== null ? 200 : 0;
      setTimeout(() => {
        this.open();
      }, fixForInput);
    });
    ...
    EventHandler$1.on(
      ...
      ({ target }) => {
        ...
        if (Manipulator$1.hasClass(target, BUTTON_CLEAR_CLASS)) {
          this._toggleAmPm("PM");
          this._input.value = "";
          EventHandler$1.trigger(this._input, EVENT_CLEAR);
          Manipulator$1.removeClass(this._input, "active");
          let checkInputValue;
  ...
  _handleOkButton() {
    EventHandlerMulti.on(this._modal, "click", `.${BUTTON_SUBMIT_CLASS}`, () => {
      ...
      if (this._isInvalidTimeFormat) {
        Manipulator$1.removeClass(this._input, "is-invalid");
      }
      if (format24) {
      ...
      this.close();
      EventHandler$1.trigger(this._input, EVENT_VALUE_CHANGED$4);
    });
  }
...
class Navbar extends BaseComponent2 {
  ...
  static jQueryInterface(config) {
    this.each(function() {
      const data = Navbar.getOrCreateInstance(this);
      if (typeof config !== "string") {
        return;
      }
      if (typeof data[config] === "undefined") {
...
function isFullYearFormat(format) {
  return format == null ? void 0 : format.toLowerCase().includes("yyyy");
}
...
function isInRange(date, range) {
  if (range.length === 2) {
    return compareDates(date, range[0]) > 0 && compareDates(date, range[1]) < 0;
  }
  return false;
}
function isEdgeOfRange(date, range) {
  if (range.length === 2) {
    if (isSameDate(range[0], range[1]))
      return false;
    if (isSameDate(date, range[0]))
      return "first";
    if (isSameDate(date, range[1]))
      return "last";
  }
  return false;
}
...
function getDatepickerTemplate(date, selectedDate, selectedYear, selectedMonth, options, monthsInRow, yearsInView, yearsInRow, id, selectedRange) {
...
`;
  ...
  if (options.dateRange) {
    Manipulator$1.addClass(template, "datepicker-date-range");
  }
  return template;
}
...
function createTitle(selected, selectedRange, options) {
...
function createMainContent(date, month, year, selectedDate, selectedYear, selectedMonth, options, monthsInRow, yearsInView, yearsInRow, selectedRange) {
...
function createViewTemplate(date, year, selectedDate, selectedYear, selectedMonth, options, monthsInRow, yearsInView, yearsInRow, selectedRange) {
  let viewTemplate;
...
function createDayViewTemplate(date, headerDate, options, selectedRange) {
  const dates = getDatesArray(date, headerDate, options, selectedRange, 1);
  const nextMonthDates = options.dateRange ? getDatesArray(addMonths(date, 1), headerDate, options, selectedRange, 2) : null;
  ...
  const tableHeadContent = `
    ...
    `;
  const tableBodyContent = (dates2) => dates2.map(
    (week) => `
          <tr>
            ${week.map((day) => {
      ...
      const currentDate = `${getYear$1(day.date)}-${getMonth$1(day.date)}-${getDate(
        day.date
      )}`;
  ...
  const dayViewTemplate = (dates2, month) => `
    <table class="datepicker-table">
        ...
        <tbody class="datepicker-table-body">
         ${tableBodyContent(dates2)}
        </tbody>
      </table>
    `;
  const rangeTemplate = () => `
    ${dayViewTemplate(dates, getMonth$1(date))}
    <div class="vr"></div>
    ${dayViewTemplate(nextMonthDates, getMonth$1(addMonths(date, 1)))}
    `;
  return options.dateRange ? rangeTemplate() : dayViewTemplate(dates, getMonth$1(date));
}
function getDatesArray(activeDate, headerDate, options, selectedRange = [], visibleMonth) {
  ...
  const isVisible2 = options.dateRange ? month === firstVisibleMonth || month === secondVisibleMonth : month === firstVisibleMonth;
  const isSelected = (date, selectedRange2) => {
    if (!selectedRange2 || (selectedRange2 == null ? void 0 : selectedRange2.length) === 0)
      return false;
    if (options.dateRange) {
      return selectedRange2.length === 1 ? isSameDate(date, selectedRange2[0]) : isSameDate(date, selectedRange2[0]) || isSameDate(date, selectedRange2[1]);
    }
    return isSameDate(date, selectedRange2);
  };
    ...
    if (i === 1) {
      ...
      for (let j = 0; j < daysLeft; j++) {
        ...
        week.push({
          date,
          currentMonth: isCurrentMonth,
          isSelected: isSelected(date, headerDate) && isVisible2,
          isInRange: isInRange(date, selectedRange) && isCurrentMonth,
          isEdgeOfRange: isCurrentMonth ? isEdgeOfRange(date, selectedRange) : false,
          isToday: isSameDate(date, getToday()),
          dayNumber: getDate(date),
...
function getToggleButtonTemplate(id, ariaLabel) {
  return `
...
class Datepicker extends BaseComponent2 {
  ...
  get visibleDates() {
    return SelectorEngine$1.find("td", this.datesContainer);
  }
  update(options = {}) {
    this._options = this._getConfig({ ...this._options, ...options });
  }
  ...
  _appendToggleButton() {
    const toggleButton = getToggleButtonTemplate(
      ...
      this._options.toggleButtonLabel
    );
  ...
  _listenToOutsideClick() {
    EventHandler$1.on(document, EVENT_CLICK_DATA_API, (e) => {
      ...
      const isContainerContent = (_a = this.container) == null ? void 0 : _a.contains(e.target);
  ...
  _handleDaysViewKeydown(event) {
    ...
    const isDateInSameView = areDatesInSameView(oldActiveDate, this._activeDate, this._view) || this._options.dateRange && areDatesInSameView(addMonths(this._firstVisibleMonth, 1), this._activeDate, this._view);
    if (!isDateInSameView) {
      this._changeView("days");
    }
  ...
  _selectDate(date, cell = this.activeCell) {
    const { min: min2, max: max2, filter, disablePast, disableFuture, inline, confirmDateOnSelect, dateRange } = this._options;
    if (isDateDisabled(date, min2, max2, filter, disablePast, disableFuture)) {
      return;
    }
    this._removeCurrentFocusStyles();
    dateRange ? this._handleDateRangeSelection(date, cell) : this._handleSingleDateSelection(date, cell);
  ...
  _addDateRangeStyles() {
    ...
    this.visibleDates.forEach((cell) => {
      ...
      if (isSameDate(cellDate, firstDate)) {
        Manipulator$1.addClass(cell, "first-in-range");
      }
      if (isSameDate(cellDate, lastDate)) {
        Manipulator$1.addClass(cell, "last-in-range");
      }
      if (isInRange(cellDate, [firstDate, lastDate])) {
        Manipulator$1.addClass(cell, "in-range");
      }
    });
  }
  _confirmSelection(date) {
    if (date) {
      ...
      Manipulator$1.addClass(this._input, "active");
      if (this.isSelectionCompleted) {
        EventHandler$1.trigger(this._element, EVENT_VALUE_CHANGED$3, {
  ...
  _removeCurrentStyle(style) {
    const currentSelected = SelectorEngine$1.find(`.${style}`, this.container);
    if (currentSelected) {
  ...
  nextMonth() {
    const nextMonth = this.options.dateRange ? addMonths(this._firstVisibleMonth, 1) : addMonths(this._activeDate, 1);
  ...
  previousMonth() {
    const previousMonth = this.options.dateRange ? addMonths(this._firstVisibleMonth, -1) : addMonths(this._activeDate, -1);
  ...
  _handleUserInput(input) {
    ...
    if (this.options.dateRange) {
      dates = input.split(" - ");
      dates = dates.map((date2) => this._parseDate(date2, this._options.format, delimeters));
      firstDateValid = isValidDate$1(dates[0]);
      secondDateValid = (dates == null ? void 0 : dates.length) > 1 ? isValidDate$1(dates[1]) : false;
    ...
    if (isValidDate$1(date) && !this.options.dateRange) {
      this._setDate(date);
    } else if (firstDateValid && secondDateValid && areDatesChronological) {
      this._setDate(dates);
    } else {
      this._activeDate = this._options.startDate ? new Date(this._options.startDate) : /* @__PURE__ */ new Date();
      this._selectedRange = [];
      this._selectedDate = null;
      this._selectedMonth = null;
      this._selectedYear = null;
  ...
  _setDate(dateOrDates) {
    if (this.options.dateRange) {
      ...
      this._selectedYear = [getYear$1(startDate), getYear$1(endDate)];
      this._selectedMonth = [getMonth$1(startDate), getMonth$1(endDate)];
      this._selectedRange = [startDate, endDate];
    } else {
      this._activeDate = dateOrDates;
      this._selectedDate = dateOrDates;
      this._selectedYear = getYear$1(dateOrDates);
      this._selectedMonth = getMonth$1(dateOrDates);
...
const columns = (columns2, selectable, multi, columnSearch) => {
...
` : '<th scope="col"></th>';
  const headers = columns2.map((column, i) => {
    ...
    return `<th style="${column.fixed ? `${column.fixed === "right" ? "right" : "left"}: ${fixedOffset}px;` : ""}" ${column.fixed ? 'class="fixed-cell"' : ""} scope="col">
    <div>${column.sort ? `<i data-mdb-sort="${column.field}" class="datatable-sort-icon fas fa-arrow-up"></i>` : ""}
    ${column.label} </div>
    ${columnSearch ? `<div class="datatable-header-filter"><input type="text" data-mdb-field="${column.field}" class="datatable-header-filter-input" /></div>` : ""}
    </th>`;
  });
...
const tableTemplate = ({
  ...
  const columnsTemplate = columns(columns$1, selectable, multi, columnSearch);
  const table = `
...
const pagination = ({ entries, entriesOptions, fullPagination, rowsText, allText }, loading, text) => {
...
const createEntriesSelect = (entries, entriesOptions, loading, rowsText, allText) => {
  ...
  return `
    <div class="datatable-select-wrapper">
      ...
      <select name="entries" ${loading ? 'data-mdb-disabled="true"' : ""} class="datatable-select select">
        ${options}
      </select>
    </div>
  `;
...
function getWrapperTemplate(id, config, label, dropdownContainerId, inputId) {
  ...
  input.setAttribute("aria-controls", dropdownContainerId);
  ...
  clearBtn.setAttribute("aria-label", config.clearButtonLabel);
...
function getOptionsListTemplate(options, selectAllOption, config, label) {
  const optionsList = element("div");
  Manipulator$1.addClass(optionsList, "select-options-list");
  if (label == null ? void 0 : label.id) {
  ...
  optionsList.setAttribute("role", role);
  optionsList.setAttribute("aria-multiselectable", multiselectable);
  let optionsNodes;
  if (config.multiple) {
...
class Select extends BaseComponent2 {
  constructor(element2, config) {
    ...
    this._inputId = this._element.id ? `select-input-${this._element.id}` : getUID$1("select-input-");
  ...
  _handleClosedKeydown(event) {
    ...
    const isOpenKey = key === ENTER || key === SPACE || (key === DOWN_ARROW || key === UP_ARROW) && event.altKey || key === DOWN_ARROW && this.multiple;
    if (isOpenKey) {
      this.open();
    }
    if (!this.multiple) {
  ...
  _updateClearButtonVisibility() {
    ...
    if (((_a = this._optionsToRender[0]) == null ? void 0 : _a.hidden) && ((_b = this._optionsToRender[0]) == null ? void 0 : _b.selected) || !this.hasSelection) {
  ...
  _listenToSelectSearch() {
    EventHandler$1.on(this.filterInput, "input", this._inputHandler);
  }
  _handleInput(event) {
  ...
  close() {
    ...
    if (this._config.filter) {
      EventHandler$1.off(this.filterInput, "input", this._inputHandler);
    }
  ...
  setValue(value) {
    if (this._config.filter && this.filterInput.value) {
      this._resetFilterState();
      EventHandler$1.trigger(this.filterInput, "input");
    }
    this._optionsToRender = this._getOptionsToRender(this._element);
    this._plainOptions = this._getPlainOptions(this._optionsToRender);
  ...
  _selectByValue(value) {
    const option = this.options.find((option2) => option2.value === value);
    if (option) {
      option.select();
      if (option.nativeOption) {
...
const SELECTOR_PAGINATION$1 = ".datatable-pagination";
...
const EVENT_PAGINATION_CHANGE$1 = "paginationChange.mdb.datatable";
const EVENT_VALUE_CHANGED_SELECT$1 = "valueChanged.mdb.select";
class DatatablePagination {
  ...
  get navigationText() {
    const entries = this._optionToLowerCase(this._options.entries) === "all" ? this._total : this._entries;
  ...
  removePagination() {
    ...
    this._removeEventListeners();
    const paginationEl = SelectorEngine$1.findOne(SELECTOR_PAGINATION$1, this._element);
    if (paginationEl) {
      paginationEl.remove();
    }
    this._pagination = null;
  ...
  // Private
  _setNavigationText() {
    const nav = SelectorEngine$1.findOne(SELECTOR_PAGINATION_NAV, this._pagination);
    nav.innerHTML = this.navigationText;
  }
  _setSelectLabel() {
    const selectLabel = SelectorEngine$1.findOne(SELECTOR_SELECT_LABEL, this._pagination);
  ...
  _updatePaginationButtonsTemplate() {
    ...
    const paginationButtons = SelectorEngine$1.findOne(SELECTOR_PAGINATION_BUTTONS, this._pagination);
    paginationButtons.outerHTML = buttonsHTML;
  }
  _setPaginationValues() {
    const { pagination: pagination2 } = this._options;
    this._activePage = pagination2.page || 0;
    this._total = pagination2.total || 0;
    this._entries = pagination2.entries || this._options.entries;
  }
  _appendPagination() {
    ...
    this._pagination = SelectorEngine$1.findOne(SELECTOR_PAGINATION$1, this._element);
  }
  ...
  _removeEventListeners() {
    EventHandler$1.off(this._right, "click");
    EventHandler$1.off(this._left, "click");
    if (this._options.fullPagination) {
      EventHandler$1.off(this._start, "click");
      EventHandler$1.off(this._end, "click");
    }
  }
  _setupPagination() {
    this._right = SelectorEngine$1.findOne(SELECTOR_PAGINATION_RIGHT, this._element);
    this._left = SelectorEngine$1.findOne(SELECTOR_PAGINATION_LEFT, this._element);
    const paginationButtons = [
      { button: this._right, action: () => this._activePage += 1 },
      { button: this._left, action: () => this._activePage -= 1 }
    ];
    if (this._options.fullPagination) {
      this._start = SelectorEngine$1.findOne(SELECTOR_PAGINATION_START, this._element);
      this._end = SelectorEngine$1.findOne(SELECTOR_PAGINATION_END, this._element);
    ...
    paginationButtons.forEach(({ button, action }) => {
      if (button) {
        EventHandler$1.on(button, "click", () => {
          action();
          EventHandler$1.trigger(this._element, EVENT_PAGINATION_CHANGE$1, {
  ...
  _setupPaginationSelect() {
    ...
    EventHandler$1.on(this._select, EVENT_VALUE_CHANGED_SELECT$1, (e) => {
  ...
  _disposePaginationSelect() {
    EventHandler$1.off(this._select, EVENT_VALUE_CHANGED_SELECT$1);
    if (this._selectInstance) {
      this._selectInstance.dispose();
    }
  }
...
class Datatable extends BaseComponent2 {
  constructor(element2, data = {}, options = {}) {
    ...
    this._pagination = new DatatablePagination(element2, this._options);
    this._selected = [];
  ...
  get computedRows() {
    ...
    if (this._options.pagination) {
      const entries = this._optionToLowerCase(this._options.entries) === "all" ? result.length : this._options.entries;
      result = paginate({
        rows: result,
        entries,
  ...
  _enforceOptionDependency() {
    if (this._options.serverSide && this._options.pagination && typeof this._options.pagination !== "object") {
      console.warn(
  ...
  _setupPagination() {
    const paginationEl = SelectorEngine$1.findOne(SELECTOR_PAGINATION, this._element);
    if (!paginationEl) {
  ...
  _setupEditable() {
    SelectorEngine$1.find(SELECTOR_ROW, this._element).forEach((row) => {
      ...
      SelectorEngine$1.find(SELECTOR_CELL, row).forEach((cell) => {
        ...
        EventHandler$1.on(cell, "focus", () => {
          cell.classList.add("editable-active");
        });
        EventHandler$1.on(cell, "blur", () => {
  ...
  _setupSort() {
    SelectorEngine$1.find(SELECTOR_SORT_ICON, this._element).forEach((icon) => {
      ...
      const [header] = SelectorEngine$1.parents(icon, "div");
      ...
      EventHandler$1.on(header, "click", () => {
        ...
        EventHandler$1.trigger(this._element, EVENT_SORT_CHANGE, {
          sorting: {
            field,
  ...
  _setupColumnSearch() {
    this._columnSearchInputs = SelectorEngine$1.find(SELECTOR_HEADER_FILTER_INPUT, this._element);
    this._columnSearchInputs.forEach((input) => {
      EventHandler$1.on(input, "input", (e) => {
        const value = e.target.value;
        const field = Manipulator$1.getDataAttribute(input, "field");
        const columnIndex = this._getColumnIndex(field);
        EventHandler$1.trigger(this._element, EVENT_COLUMN_SEARCH, {
          value,
          field,
          columnIndex
        });
  ...
  _removeEventListeners() {
    if (this._options.pagination) {
      EventHandler$1.off(this._element, EVENT_PAGINATION_CHANGE);
      EventHandler$1.off(this._pagination._select, EVENT_VALUE_CHANGED_SELECT);
    }
    ...
    if (this._options.editable) {
      SelectorEngine$1.find(SELECTOR_CELL, this._element).forEach((cell) => {
        ...
        EventHandler$1.off(cell, "focus");
        EventHandler$1.off(cell, "blur");
      });
    }
    if (this._options.clickableRows) {
      SelectorEngine$1.find(SELECTOR_ROW, this._element).forEach((row) => {
    ...
    if (this._options.columnSearch) {
      this._columnSearchInputs.forEach((input) => {
        EventHandler$1.off(input, "input");
      });
    }
  }
  _renderTable() {
  ...
  _setupPaginationListeners() {
    ...
    EventHandler$1.on(this._element, EVENT_PAGINATION_CHANGE, (e) => {
      var _a;
      if (this._activePage !== ((_a = e.pagination) == null ? void 0 : _a.page)) {
        this._changeActivePage(e.pagination.page);
      }
    });
    EventHandler$1.on(this._pagination._select, EVENT_VALUE_CHANGED_SELECT, (e) => {
...
class Pan extends TouchUtil {
  ...
  handleTouchEnd(e) {
    ...
    this._startTouch = null;
    EventHandler$1.trigger(this._element, EVENT_END$3, { touch: e });
  }
}
...
class Pinch extends TouchUtil {
  ...
  handleTouchEnd(e) {
    if (this.isNumber) {
      this._startTouch = null;
      EventHandler$1.trigger(this._element, EVENT_END$2, {
        touch: e,
        ratio: this._ratio,
        origin: this._origin
      });
...
class Rotate extends TouchUtil {
  ...
  handleTouchEnd(e) {
    e.type === "touchend" && e.preventDefault();
    this._origin = {};
    EventHandler$1.trigger(this._element, EVENT_END$1, { touch: e });
  }
}
const NAME$b = "touch";
  ...
  _handleOpenKeydown(event) {
    ...
    switch (key) {
      ...
      case HOME:
        ...
        } else if (((_a = this._filteredResults) == null ? void 0 : _a.length) > 0 && this._isOpen) {
      ...
      case END:
        if (event.shiftKey) {
          ...
          this._input.setSelectionRange(start2, end2);
        } else if (((_b = this._filteredResults) == null ? void 0 : _b.length) > 0 && this._isOpen) {
...
const EVENT_END = `end${EVENT_KEY$1}`;
...
class MultiRangeSlider extends BaseComponent2 {
  ...
  get handActive() {
    return SelectorEngine$1.findOne(`${CLASSNAME_HAND}.active`, this._element);
  }
  get activeTooltip() {
    return SelectorEngine$1.findOne(CLASSNAME_TOOLTIP, this._element);
  }
  get activeTooltipValue() {
    const handTooltip = SelectorEngine$1.findOne(`${CLASSNAME_HAND}.active`, this._element);
    return SelectorEngine$1.findOne(`${CLASSNAME_TOOLTIP_VALUE}`, handTooltip);
  }
  // Public
  init() {
  ...
  dispose() {
    Manipulator$1.removeDataAttribute(this._element, `${this.constructor.NAME}-initialized`);
    Manipulator$1.removeClass(this._element, SELECTOR_HORIZONTAL);
    this._removeEventListeners();
  ...
  _handleClickEventOnHand(ev) {
    ...
    this.hands.forEach((h) => {
      Manipulator$1.addStyle(h, { zIndex: h === hand ? "10" : "1" });
    });
  ...
  _handleMouseMoveEventDocument(ev) {
    ...
    if (Manipulator$1.hasClass(hand, SELECTOR_ACTIVE)) {
      ...
      if (step !== null) {
        ...
        const normalizedSnappedValue = (snappedValue - min2) / (max2 - min2);
      ...
      if (numberOfRanges >= 2) {
        for (const otherHand of this.handsNoActive) {
          const otherTranslation = parseFloat(
            Manipulator$1.getDataAttribute(otherHand, "translation")
          );
          const otherHandle = parseInt(Manipulator$1.getDataAttribute(otherHand, "handle"), 10);
          const currentHandle = parseInt(Manipulator$1.getDataAttribute(hand, "handle"), 10);
  ...
  _handleEndMoveEventDocument() {
    ...
    if (this._mousemove) {
      ...
      this.hands.forEach((hand2) => {
        Manipulator$1.removeClass(hand2, SELECTOR_ACTIVE);
        if (this._options.tooltip)
          Manipulator$1.removeClass(hand2.children[1], "active");
      });
      this._mousemove = false;
    }
    EventHandler$1.trigger(hand, EVENT_END, { hand });
    this._movingHand = null;
  ...
  _handleClickOnRange(ev) {
    this.hands.forEach((hand) => {
      ...
      if (this._options.numberOfRanges < 2) {
        ...
        Manipulator$1.addClass(firstHand, SELECTOR_ACTIVE);
        const translation = getEventTypeClientX(ev) - this.leftConnectRect - hand.offsetWidth / 2;
        const currentHandle = parseInt(Manipulator$1.getDataAttribute(firstHand, "handle"), 10);
        let canMove = true;
        ...
        if (canMove) {
          Manipulator$1.addStyle(firstHand, {
            transform: `translate(${translation}px,-25%)`
          });
          Manipulator$1.setDataAttribute(firstHand, "translation", translation);
        }
      }
    });
  }
...
const EVENT_VALUE_CHANGED_DATEPICKER = "valueChanged.mdb.datepicker";
...
class Datetimepicker extends BaseComponent2 {
  ...
  _openDatePicker() {
    ...
    if (!this._options.inline) {
      this._scrollBar.hide();
      Manipulator$1.addClass(this._datepicker.container, "datetimepicker-datepicker-modal");
    }
    if (this._options.inline || this._options.datepicker.inline) {
  ...
  _handleTimepickerDisablePast() {
    const currentDate = /* @__PURE__ */ new Date();
    currentDate.setHours(0, 0, 0, 0);
    EventHandler$1.on(this._datepicker._element, EVENT_VALUE_CHANGED_DATEPICKER, () => {
  ...
  _openTimePicker() {
    EventHandler$1.trigger(this._timepicker.toggleButton, "click");
    setTimeout(() => {
      this._addIconButtons();
      if (this._options.inline || this._options.timepicker.inline) {
...
const lightboxCallback = (component, initSelector) => {
  ...
  const SELECTOR_DATA_TOGGLE2 = `[data-mdb-${component.NAME}-initialized]`;
  const SELECTOR_TOGGLE2 = `${SELECTOR_DATA_TOGGLE2} img:not(.lightbox-disabled)`;
  const Lightbox2 = component;
  SelectorEngine$1.find(initSelector).forEach((element2) => {
    return Lightbox2.getOrCreateInstance(element2);
  });
  if (callbackInitState.has(component.name)) {
    return;
  }
  EventHandler$1.on(document, EVENT_CLICK_DATA_API2, SELECTOR_TOGGLE2, (event) => {
    event.preventDefault();
    Lightbox2.toggle(event);
  });
  callbackInitState.set(component.name, true);
};
